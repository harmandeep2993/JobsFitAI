...
async def api_ats_check(
    ...
    record = resume_store.get(current_user["id"], resume_id)
    if not record:
...
async def api_ats_optimise(
    ...
    Accepts {resume_id, jd}. Returns a complete ATS-optimised resume with
    coverage before/after, section flags, and formatting warnings.
    """
    resume_id = (body.resume_id or "").strip()
    jd_text = (body.jd or "").strip()
