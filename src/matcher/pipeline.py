...
def get_match_score(resume_json, jd_json):
    ...
    req_score, matched_req, missing_req = (
        score_required_skills(resume_json, jd_json)
    )

    print("Scoring responsibilities semantically...")
    resp_score = score_responsibilities(
        resume_json, jd_json
    )
    ...
    pref_score, matched_pref, missing_pref = (
        score_preferred_skills(resume_json, jd_json)
    )
